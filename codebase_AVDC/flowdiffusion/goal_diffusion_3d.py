...
class GoalGaussianDiffusion(nn.Module):
    ...
    def get_tiled_x(self, x, x_cond):
        ...
        n = x.shape[1] // 3
        ### repeat x_cond
        x_cond = x_cond.unsqueeze(2).repeat(1, 1, n, 1, 1) # (b, 3, n, h, w)
        x = rearrange(x, 'b (n c) h w -> b c n h w', n=n) # (b, 3, n, h, w)
        return torch.cat([x, x_cond], dim=1) # (b, 6, n, h, w)


    def model_predictions(self, x, t, x_cond, task_embed,  clip_x_start=False, rederive_pred_noise=False, cond_scale=1.):
        # task_embed = self.text_encoder(goal).last_hidden_state
        ### x: (b, n*3, h, w) -> (b, 3, n, h, w)
        model_output = self.model.forward_with_cond_scale(rearrange(x, 'b (n c) h w -> b c n h w', c=3), t, x_cond, task_embed, cond_scale=cond_scale)
        model_output = rearrange(model_output, 'b c n h w -> b (n c) h w')
    ...
    def p_mean_variance(self, x, t, x_cond, task_embed,  clip_denoised = False, cond_scale=1.):
        preds = self.model_predictions(x, t, x_cond, task_embed, cond_scale=cond_scale)
    ...
    @torch.no_grad()
    def p_sample(self, x, t: int, x_cond, task_embed, cond_scale=1.):
        b, *_, device = *x.shape, x.device
        batched_times = torch.full((b,), t, device = x.device, dtype = torch.long)
        model_mean, _, model_log_variance, x_start = self.p_mean_variance(x, batched_times, x_cond, task_embed, clip_denoised = True, cond_scale=cond_scale)
        noise = torch.randn_like(x) if t > 0 else 0. # no noise if t == 0
        pred_img = model_mean + (0.5 * model_log_variance).exp() * noise
        return pred_img, x_start

    @torch.no_grad()
    def p_sample_loop(self, shape, x_cond, task_embed, return_all_timesteps=False, cond_scale=1.):
        ...
        for t in tqdm(reversed(range(0, self.num_timesteps)), desc = 'sampling loop time step', total = self.num_timesteps):
            ...
            img, x_start = self.p_sample(img, t, x_cond, task_embed, cond_scale=cond_scale)
            imgs.append(img)
    ...
    @torch.no_grad()
    def ddim_sample(self, shape, x_cond, task_embed, return_all_timesteps=False, cond_scale=1.):
        ...
        for time, time_next in tqdm(time_pairs, desc = 'sampling loop time step'):
            ...
            # self_cond = x_start if self.self_condition else None
            pred_noise, x_start, *_ = self.model_predictions(img, time_cond, x_cond, task_embed, clip_x_start = False, rederive_pred_noise = True, cond_scale=cond_scale)
    ...
    @torch.no_grad()
    def sample(self, x_cond, task_embed, batch_size = 16, return_all_timesteps = False, cond_scale=1.):
        image_size, channels = self.image_size, self.channels
        sample_fn = self.p_sample_loop if not self.is_ddim_sampling else self.ddim_sample
        return sample_fn((batch_size, channels, image_size[0], image_size[1]), x_cond, task_embed,  return_all_timesteps = return_all_timesteps, cond_scale=cond_scale)
    ...
    def p_losses(self, x_start, t, x_cond, task_embed, noise=None):
        ...
        x = rearrange(x, 'b (n c) h w -> b c n h w', c=3)
        model_out = self.model(x, t, x_cond, task_embed, cond_drop_prob=self.cond_drop_prob)
        ### output shape: (b, c, n, h, w) -> (b, n*c, h, w)
        model_out = rearrange(model_out, 'b c n h w -> b (n c) h w')

        # print(model_out.max().detach().item(), model_out.min().detach().item())
...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        super().__init__()
        self.sample_cond_scale = sample_cond_scale
        ...
        self.opt = Adam(diffusion_model.parameters(), lr = train_lr, betas = adam_betas, eps = 1e-4)
    ...
    def train(self):
        ...
        with tqdm(initial = self.step, total = self.train_num_steps, disable = not accelerator.is_main_process) as pbar:
            ...
            while self.step < self.train_num_steps:
                ...
                # accelerator.clip_grad_value_(self.model.parameters(), 10.0)
                ...
                scale = self.accelerator.scaler.get_scale()
                    ...
                    ### get maximum and min gradient magnitude
                    # max_grad = 0
                    # min_grad = 0
                    ...
                    # self.accelerator.print(f'max_grad: {max_grad:.4E}, min_grad: {min_grad:.4E}') 

                pbar.set_description(f'loss: {total_loss:.4E}, loss scale: {scale:.1E}')
                # pbar.set_description(f'loss: {total_loss:.4E} loss scale: {scale:.1E} max_grad: {max_grad:.4E}, min_grad: {min_grad:.4E}')
                ### just print them
                # self.accelerator.print(f'loss: {total_loss:.4E} max_grad: {max_grad:.4E}, min_grad: {min_grad:.4E}')
                ...
                if accelerator.is_main_process:
                    ...
                    if self.step != 0 and self.step % self.save_and_sample_every == 0:
                        ...
                        with torch.no_grad():
                            ...
                            with self.accelerator.autocast():
                                all_xs_list = list(map(lambda n, c, e: self.ema.ema_model.sample(batch_size=n, x_cond=c, task_embed=e, cond_scale=self.sample_cond_scale), batches, x_conds, task_embeds))
                        ...
                        if self.step == self.save_and_sample_every:
                            ...
                            gt_img = torch.cat([x_starts, x_ends, gt_xs[:, :]], dim=1)
                        ...
                        pred_img = torch.cat([x_starts, x_ends, all_xs[:, :]], dim=1)
