...
class GoalGaussianDiffusion(nn.Module):
    ...
    @torch.no_grad()
    def markovian_sample(self, x_cond, task_embed, batch_size=16, horizon=2):
        """
        predicts next frame using the previous frame as input (markovian prediction)
        repeats (horizon - 1) times
        """
        all_frames = []
        for i in tqdm(range(horizon - 1)):
            x_cond = self.sample(x_cond, task_embed, batch_size=batch_size, return_all_timesteps=False)
            all_frames.append(x_cond)
        return torch.cat(all_frames, dim=1)
        
    @torch.no_grad()
    def autoreg_sample(self, x_cond, task_embed, batch_size=16, horizon=2):
        """
        predicts next frame using all history frames as input (autoregressive prediction)
        repeats (horizon - 1) times
        """
        empty_image = torch.zeros_like(x_cond)
        if self.padding == 'r':
            all_frames = [x_cond] + [empty_image]*(horizon-2)
            for i in tqdm(range(horizon-1)):
                output = self.sample(torch.cat(all_frames, dim=1), task_embed, batch_size=batch_size, return_all_timesteps=False)
                if i+1 < len(all_frames):
                    all_frames[i+1] = output
            all_frames = all_frames[1:] + [output]
        elif self.padding == 'l':
            all_frames = [empty_image]*(horizon-2) + [x_cond]
            for i in tqdm(range(horizon-1)):
                output = self.sample(torch.cat(all_frames, dim=1), task_embed, batch_size=batch_size, return_all_timesteps=False)
                all_frames = all_frames[1:] + [output]
                ...
                if accelerator.is_main_process:
                    ...
                    if self.step != 0 and self.step % self.save_and_sample_every == 0:
                        ...
                        with torch.no_grad():
                            ...
                            all_xs_list = list(map(lambda n, c, e: self.ema.ema_model.autoreg_sample(batch_size=n, x_cond=c, task_embed=e, horizon=self.horizon), batches, x_conds, task_embeds))
